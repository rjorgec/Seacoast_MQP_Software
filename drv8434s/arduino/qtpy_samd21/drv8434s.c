...
{
    if (!dev || !dev->cfg.spi_xfer || !dev->cfg.cs) return false;
    ...
    if (tx && tx_len) memcpy(txbuf, tx, tx_len);
    ...
    for (size_t i = 0; i < rx_len; ++i) {
        if (rx[i] != 0xFF) { any_valid = true; break; }
    }
...
{
    if (!dev || !out || out_len == 0) return false;
    ...
    bool any_valid = false;
    for (size_t i = 0; i < out_len; ++i) {
        if (out[i] != 0xFF) { any_valid = true; break; }
    }
